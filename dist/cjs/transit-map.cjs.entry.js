...
const TransitMap = class {
    ...
    initializeMap() {
        esriLoader.esriLoader_3(['esri/views/MapView', 'esri/Map', 'esri/layers/GraphicsLayer', 'esri/layers/FeatureLayer', 'esri/widgets/Locate']).then(([MapView, Map, GraphicsLayer, FeatureLayer, Locate]) => {
            const now = new Date();
    ...
    addVehiclesToMap(vehicles) {
        esriLoader.esriLoader_3(['esri/geometry/Point', 'esri/Graphic']).then(([Point, Graphic]) => {
    ...
    addRouteToMap(paths) {
        esriLoader.esriLoader_3(['esri/geometry/Polyline', 'esri/Graphic']).then(([Polyline, Graphic]) => {
    ...
    static get style() { return "\@import url(\"https://js.arcgis.com/4.13/esri/themes/light/main.css\");.container,body,html{height:100%;width:100%;margin:0;padding:0}.esri-view{height:50%;width:100%}.stops{height:50%}"; }
};

exports.transit_map = TransitMap;
