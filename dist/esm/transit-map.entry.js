...
const TransitMap = class {
    ...
    initializeMap() {
        esriLoader_3(['esri/views/MapView', 'esri/Map', 'esri/layers/GraphicsLayer', 'esri/layers/FeatureLayer', 'esri/widgets/Locate']).then(([MapView, Map, GraphicsLayer, FeatureLayer, Locate]) => {
            const now = new Date();
    ...
    addVehiclesToMap(vehicles) {
        esriLoader_3(['esri/geometry/Point', 'esri/Graphic']).then(([Point, Graphic]) => {
    ...
    addRouteToMap(paths) {
        esriLoader_3(['esri/geometry/Polyline', 'esri/Graphic']).then(([Polyline, Graphic]) => {
