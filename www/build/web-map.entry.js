import { r as registerInstance, h, d as getElement } from './core-17370b86.js';
import { e as esriLoader_3, a as esriLoader_5 } from './esri-loader-42a97741.js';
...
const WebMap = class {
    ...
    async initializeMap() {
        try {
            const [WebMap, Map, MapView] = await esriLoader_3([
                'esri/WebMap',
                'esri/Map',
                'esri/views/MapView'
            ]);
    ...
    async initializeScene() {
        try {
            const [WebMap, WebScene, SceneView] = await esriLoader_3([
                'esri/WebMap',
                'esri/WebScene',
    ...
    async initializeSearch(mapView) {
        try {
            const [Search] = await esriLoader_3([
                'esri/widgets/Search'
            ]);
    ...
    async initializeLayerList(mapView) {
        try {
            const [LayerList, Expand] = await esriLoader_3([
    ...
    async initializeBaseMapGallery(mapView) {
        try {
            const [BasemapGallery, Expand] = await esriLoader_3([
    ...
    async initializeLegend(mapView) {
        try {
            const [Legend, Expand] = await esriLoader_3([
                'esri/widgets/Legend',
                'esri/widgets/Expand'
            ]);
